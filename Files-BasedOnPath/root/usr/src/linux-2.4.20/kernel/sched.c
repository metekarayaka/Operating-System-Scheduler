...
repeat_schedule:
	...
	if (SCHED_RAFFLE == 0){  /* Prototype Added */
	...
	else if (SCHED_RAFFLE == 1) {  /* Prototype Added (This is our custom scheduler)*/		
		int sumOfTickets;
		int selectedFlag;
		uid_t userID;
	repeat_process:
		/* WE SELECT THE LUCKY USER */
		selectedFlag = -1;
		list_for_each(tmp, &runqueue_head) {
			p = list_entry(tmp, struct task_struct, run_list);
			if (p->user->flag == 0){
		...
		list_for_each(tmp, &runqueue_head) {
			p = list_entry(tmp, struct task_struct, run_list);
			if ((p->numberOfTickets < 15) && ((jiffies - p->lastJiffies) > 12)){
				p->numberOfTickets = p->numberOfTickets +1;			
			}
			else if ((p->numberOfTickets > 1) && ((jiffies - p->lastJiffies) < 4)){
				p->numberOfTickets = p->numberOfTickets -1;
			}
			if (p->user->uid == userID){
				sumOfTickets = sumOfTickets + p->numberOfTickets;
			}
		}
		int rand = 0;
		int prizeTicket = (get_random_bytes(&rand, sizeof(int)) % sumOfTickets) + 1;
		if (prizeTicket < 0){
			prizeTicket = prizeTicket * -1;
		}
		...
		list_for_each(tmp, &runqueue_head){	
			p = list_entry(tmp, struct task_struct, run_list);
			if(p->user->uid == userID){
				prizeTicket = prizeTicket - p->numberOfTickets;
				if (prizeTicket <= 0){
					next = p; // THE NEXT PROCESS SCHEDULER WILL DISPATCH (RUN)
					p->lastJiffies = jiffies;
					break;
				}
			}
		}
	...
	/*p = list_entry(tmp, struct task_struct, run_list);
			if ((p->numberOfTickets < 15) && ((jiffies - p->lastJiffies) > 12) && (p->user->uid == userID)){
				p->numberOfTickets = p->numberOfTickets +1 ;
				sumOfTickets = sumOfTickets + p->numberOfTickets;			
			}
			else if ((p->numberOfTickets > 1) && ((jiffies - p->lastJiffies) < 4) && (p->user->uid == userID)){
				p->numberOfTickets = p->numberOfTickets -1;
				sumOfTickets = sumOfTickets + p->numberOfTickets;
			}
			...
			/*if ((p->numberOfTickets < 15) && ((jiffies - p->lastJiffies) > 12)){
				...
				/*if (prizeTicket <= p->numberOfTickets){
					p->lastJiffies = jiffies;
					next = p; // THE NEXT PROCESS SCHEDULER WILL DISPATCH (RUN)
					break;
				}*/
